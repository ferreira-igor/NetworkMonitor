...
String browseServices(const char *service, const char *protocol) {
  ...
  if (num_services != 0) {
    for (int i = 0; i < num_services; ++i) {
      ...
      browse_services += "\n";
      browse_services += "IP: ";
      browse_services += MDNS.address(i).toString();
      browse_services += ":";
...
void setup() {
  ...
  while (!LittleFS.begin(format_littlefs_if_failed)) {
    Serial.println("Trying to mount file system...");
    delay(1000);
  }
  ...
  while (!MDNS.begin(ap_name)) {
    Serial.println("Trying to start mDNS service...");
    delay(1000);
  }
  ...
  while (!bot.begin()) {
    Serial.println("Trying to connect to Instagram bot...");
    delay(1000);
  }

  const char *welcome_message = "NetworkMonitor is online!"
                                "\n\n"
                                "Type /help to show all available commands.";
  bot.sendTo(atoi(chat_id), welcome_message);
...
void loop() {
  ...
  if (bot.getNewMessage(bot_msg)) {
    ...
    if (recv_msg.equals("/help")) {
      ...
      bot.sendMessage(bot_msg, menu);
      Serial.println(menu);
    } else if (recv_msg.equals("/mute")) {
      mute = true;
      const char *mute_true = "Bot is silenced until reboot or /unmute command.";
      bot.sendMessage(bot_msg, mute_true);
      Serial.println(mute_true);
    } else if (recv_msg.equals("/unmute")) {
      mute = false;
      const char *mute_false = "Bot is no longer silenced.";
      bot.sendMessage(bot_msg, mute_false);
      Serial.println(mute_false);
    } else if (recv_msg.equals("/services")) {
      const char *search_start = "Searching for services!"
                                 "This may take a while.";
      bot.sendMessage(bot_msg, search_start);
      ...
      const char *search_stop = "Search finished!";
      bot.sendMessage(bot_msg, search_stop);
      Serial.println(search_stop);

    } else if (recv_msg.equals("/info")) {
      ...
      info += "\n\n";
      info += "Go to https://github.com/ferreira-igor/NetworkMonitor to know more.";
      bot.sendMessage(bot_msg, info);
      Serial.println(info);
    } else {
      String bot_reply = "Hi, ";
      bot_reply += bot_msg.sender.username;
      bot_reply += "!";
      bot_reply += "\n\n";
      bot_reply += "Type /help to show all available commands.";
      bot.sendMessage(bot_msg, bot_reply);
      Serial.println(bot_reply);
    }
  }
}